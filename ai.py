...
PROMPTS = {
    ...
    "avito_anti": (
        BASE_PROMPT
        + "\n\nСгенерируй сильный пакет для продавца Авито: \
1) ответы на торг, \
2) ответы на наглые предложения уровня 'отдай за 500, если вещь стоит 1000', \
3) ответы на возражения 'дорого', 'почему так дорого', 'последняя цена?', \
4) вежливые, но твёрдые ответы без грубости, \
5) отдельный мини-блок 'когда торг уместен, а когда нет'."
    ),
    "avito_followup": (
        BASE_PROMPT
        + "\n\nТы допиливаешь уже готовое объявление для Авито. В зависимости от task верни только нужный блок.\n\n"
        + "Если task=anti: дай 10 сильных ответов на торг, включая наглые предложения типа 'вещь стоит 1000, а вам предлагают 500'.\n"
        + "Если task=ghost: дай 10 ответов пожирателям времени, пустым перепискам, просьбам придержать, бесконечным 'я подумаю'.\n"
        + "Если task=faq: дай FAQ покупателя: 7 частых вопросов и готовые ответы продавца.\n"
        + "Если task=delivery: дай 8 ответов на просьбы привезти, показать, подойти к метро, занести, отнести далеко без гарантий.\n"
        + "Если task=emoji: перепиши объявление с аккуратными эмоджи, без перебора, сохрани продающий стиль.\n"
        + "Если task=sell: перепиши объявление так, чтобы оно продавало сильнее и выглядело дороже по восприятию.\n"
        + "Ответ делай готовым к копированию."
    ),
    "youtube_pack": (
        BASE_PROMPT
